...
if __name__ == "__main__":
    ...
    path = r'D:\Program\银河证券QMT测试 - 交易终端\userdata_mini'
    # session_id为会话编号，策略使用方对于不同的Python策略需要使用不同的会话编号
    session_id = int(time.time())
    xt_trader = XtQuantTrader(path, session_id)
    # 创建资金账号为10300027082的证券账号对象
    acc = StockAccount('201130027299',xtconstant.STOCK_OPTION_ACCOUNT)
    ...
    info = xt_trader.query_account_infos()
    for i in info:
        print(i)
